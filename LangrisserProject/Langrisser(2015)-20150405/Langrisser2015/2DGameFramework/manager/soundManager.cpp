...
char* soundManager::getSoundName(string keyName)
{
	char name[128];
	...
	for (iter; iter != _mTotalSounds.end(); ++iter, count++)
	{
		if (keyName == iter->first)
		{
			iter->second[0]->getName(name, 128);
			//_sound[count]->getName(name, 128);
			break;
		}
	}
	return name;
}
int soundManager::getCurSoundPos(string keyName)
{
	unsigned int length = 0;
	arrSoundsIter iter = _mTotalSounds.begin();
	int count = 0;
...
void soundManager::setCurSoundPos(string keyName, int pos)
{
	arrSoundsIter iter = _mTotalSounds.begin();
	int count = 0;
	...
	for (iter; iter != _mTotalSounds.end(); ++iter, count++)
	{
		if (keyName == iter->first)
		{
			_channel[count]->setPosition(pos * 1000, FMOD_TIMEUNIT_MS);
			break;
		}
	}
}